...
class LeNet(nn.Module):
    def __init__(self,in_channels,args_channels,additional_layers,output_each_layer=False,linear=None,squeeze_output=True,padfirstlayer=True):
        super(LeNet,self).__init__()
        self.output_each_layer=output_each_layer
        if padfirstlayer:
            self.convs=[nn.Conv2d(in_channels,args_channels,kernel_size=5,padding=2,bias=False)]
        else:
            self.convs=[nn.Conv2d(in_channels,args_channels,kernel_size=5,padding=0,bias=False)]
